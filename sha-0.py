...
def sha0_pad_message(message: bytes) -> bytes:
    """
...
def sha0_expand_block(block: bytes) -> List[int]:
    """
    Expand 16 32-bit words into 80 32-bit words according to SHA-0 algorithm.
    """
...
def sha0_process_block(block: bytes, state: Tuple[int, ...]) -> Tuple[int, ...]:
    """
    Process a single 512-bit block according to SHA-0 algorithm.
    """
    # Constants
    K = [
...
def sha0(message: bytes) -> bytes:
    """
    Compute SHA-0 hash of a message.
    """
    # Initial state
    state = (
        0x67452301,
    ...
    # Pad message
    padded_message = sha0_pad_message(message)
    
    # Process message in 512-bit blocks
    for i in range(0, len(padded_message), 64):
        block = padded_message[i:i+64]
        state = sha0_process_block(block, state)
